...
import { ArrowLeft, User, Mail, Lock, Copy, Check, Server, Eye, EyeOff, Gamepad2 } from 'lucide-react'
...
export default function CreateClient() {
  ...
  const [clientType, setClientType] = useState('vps') // 'vps' o 'game'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post('/api/admin/clients', {
        ...
        email: email.trim(),
        password,
        mode: clientType === 'game' ? undefined : mode,
        database: clientType === 'game' ? undefined : database,
        clientType
      })
    ...
    <div className="flex-1 p-4 sm:p-8 bg-moon-bg min-h-screen">
      ...
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-moon-text/50 font-mono">Crear nuevo usuario y configurar sus servicios en moondev.online</p>
        </div>
      </div>
        ...
        {!provisionedData ? (
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  type="button"
                  onClick={() => setClientType('vps')}
                  className={`py-3 px-4 rounded-lg font-semibold text-sm border font-sans transition-all-custom flex items-center justify-center gap-2 ${
                    clientType === 'vps'
                      ? 'bg-moon-accent border-moon-accent text-white shadow-lg shadow-moon-accent/20'
                      : 'bg-moon-card border-moon-border text-moon-text hover:text-white hover:border-moon-text/30'
                  }`}
                >
                  <Server size={16} />
                  <span>VPS + Web Hosting</span>
                </button>
                <button
                  type="button"
                  onClick={() => setClientType('game')}
                  className={`py-3 px-4 rounded-lg font-semibold text-sm border font-sans transition-all-custom flex items-center justify-center gap-2 ${
                    clientType === 'game'
                      ? 'bg-moon-accent border-moon-accent text-white shadow-lg shadow-moon-accent/20'
                      : 'bg-moon-card border-moon-border text-moon-text hover:text-white hover:border-moon-text/30'
                ...
                >
                  <Gamepad2 size={16} />
                  <span>Solo Servidores de Juego</span>
                </button>
              </div>
            </div>
            ...
            >
              {loading ? (
                <div className="flex items-center gap-2">
                  ...
                  <span>{clientType === 'game' ? 'Creando cliente...' : 'Creando cliente y reservando recursos...'}</span>
                </div>
              ) : (
                'Confirmar'
              )}
          ...
          <div className="space-y-6 animate-scale-up">
            {provisionedData.instance ? (
              ...
              <>
                <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm rounded-lg flex items-center gap-2 font-mono">
                  <Gamepad2 size={18} className="animate-bounce text-moon-accent" />
                  <span>¡Cliente de juego creado con éxito!</span>
                </div>

                <div className="space-y-4">
                  <h3 className="font-bold text-white text-base">Detalles del Cliente</h3>
                  
                  <div className="bg-moon-card border border-moon-border rounded-lg p-5 space-y-3.5 text-sm font-mono">
                    <div className="flex justify-between border-b border-moon-border/40 pb-2">
                      <span className="text-moon-text/50">Usuario:</span>
                      <span className="text-white font-semibold">{provisionedData.client?.username}</span>
                    </div>
                    <div className="flex justify-between border-b border-moon-border/40 pb-2">
                      <span className="text-moon-text/50">Email:</span>
                      <span className="text-white font-semibold">{provisionedData.client?.email}</span>
                    </div>
                    <div className="flex justify-between border-b border-moon-border/40 pb-2">
                      <span className="text-moon-text/50">Tipo de Cuenta:</span>
                      <span className="text-moon-accent font-semibold">Solo Servidores de Juego</span>
                    </div>
                    <p className="text-xs text-moon-text/55 leading-relaxed pt-2">
                      Este cliente se registró de forma exitosa y no consume recursos de VPS ni puertos de hosting.
                      Ahora puedes dirigirte a la sección de **Servidores de Juego** para concederle acceso a las instancias de Valheim o Minecraft correspondientes.
                    </p>
                  </div>
                </div>
              </>
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 py-3 px-4 bg-moon-card hover:bg-moon-border/60 text-white font-semibold border border-moon-border rounded-lg text-sm transition-all-custom font-sans"
              >
                Aprovisionar Otro
              </button>
              {provisionedData.instance ? (
                <button
                  ...
                  className="flex-1 py-3 px-4 bg-moon-accent hover:bg-moon-hover text-white font-semibold rounded-lg text-sm transition-all-custom font-sans"
                >
                  Volver a la Lista
                </button>
              ) : (
                <button
                  onClick={() => navigate('/admin/game-servers')}
                  className="flex-1 py-3 px-4 bg-moon-accent hover:bg-moon-hover text-white font-semibold rounded-lg text-sm transition-all-custom font-sans"
                >
                  Ir a Servidores de Juego
                </button>
              )}
            </div>
          </div>
