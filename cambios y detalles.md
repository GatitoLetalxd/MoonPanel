...
## 📋 Registro Histórico de Cambios Recientes

### Mejoras en la Gestión de Add-ons y Estabilidad
* **Toasts personalizados no intrusivos:** Eliminación del 100% de los `alert()` y `confirm()` nativos del navegador en `GameDashboard.jsx`, reemplazándolos por un sistema propio de Toasts de notificación animados en la esquina inferior derecha.
* **Extracción recursiva con sanitización de archivos:**
  - Implementación de un extractor recursivo de archivos ZIP/mcpack/mcaddon.
  - Corrección de bugs en nombres de archivo con caracteres no estándar, unicode (ej. `❀`) o códigos de formato de color de Minecraft (ej. `§`). Los archivos temporales se renombran de manera segura antes de descomprimirse con `unzip`.
  - Migración a operaciones de sistema de archivos nativas en Node.js (`fs.cpSync` y `fs.rmSync`).
* **Resolución dinámica de nombres reales (Localización):** Los add-ons cuyo nombre en el manifest sea una clave de traducción (como `pack.name` o `resourcePack.vanilla_server.name`) ahora se resuelven leyendo los archivos `.lang` internos (`en_US.lang`, `en.lang`).
* **Modo lectura para Add-ons:** Los add-ons ahora son visibles cuando el servidor está encendido, pero las acciones de instalación, eliminación y activación de packs quedan deshabilitadas para proteger la consistencia del servidor.

### Corrección del Crash Crítico `gsl::narrowing_error`
* **Sincronización robusta de `level.dat`:**
  - Corrección de corrupción en el encabezado de 8 bytes de `level.dat`.
  - Ahora el backend calcula y sobreescribe de forma correcta el campo de tamaño del payload NBT (bytes 4-7 del header) cada vez que modifica opciones o activa experimentos del mundo. Esto previene de forma definitiva el crash fatal del servidor de Minecraft Bedrock al conectar los jugadores.
* **Experimentos integrados:** Se agregaron toggles visuales de experimentos integrados y sincronizados directamente al final de la sección de Add-ons para una administración más centralizada.
